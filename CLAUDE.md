...
- **Högskoleprovet: ORD, XYZ, MEK, KVA, NOG** (assessment prep, Swedish market) — modelled on 5 of the 8 real Högskoleprovet subtests (chose the ones that map cleanly to single-question MC solvable in 5–30s; skipped LÄS/ELF for passage length, DTK for needing images). ORD is 4 options, not the real test's 5 (see AI generation note below). KVA's 4 answers and NOG's 5 answers are always the same fixed strings per pack — generationInstructions asks the AI to balance which one is correct across the pack rather than defaulting to one. Needs `tier: "plusplus"` and `market: "se"` hand-added to the exported JSON (see Key architecture notes).
  - NOG (data sufficiency) content was authored directly rather than via the QB tool's live AI panel (which hardcodes 4 answers, incompatible with NOG's 5) and verified by exhaustive enumeration over each question's candidate domain, not by hand — sufficiency logic ("is this statement enough to pin down a unique answer") is easy to get subtly wrong. Caught and fixed several real bugs this way: contradictory statement pairs (both couldn't be true at once), an unenforced "given" fact baked into a logic-grid's intro text that never actually constrained the computation, and hand-written explanations that turned out to disagree with what the code had verified. If regenerating NOG content by hand in future, verify sufficiency claims mechanically (enumerate the candidate space) rather than reasoning it through in prose.
